...
# # Передача ссылок обычно считается самым удобным способом работы со списками и словарями, но если функция изменяет
# # переданные ей объект, нежелательно, чтобы это изменение отражалось на исходном списке или словаре. Для решения данной
# # проблемы в Python имеется модуль copy, в котором содержатся функции copy() и deepcopy(). Первая из них позволяет
# # создать копию изменяемого значения, такого как список или словарь, а не просто копию ссылки.
# import copy
# spam = ['A','B', 'C', 'D']
# print(id(spam))
...
# print(cheese)
#
# # Если копируемый список содержит вложенные списки, то в этом случае вместо функции copy.copy() следует использовать
# # функцию copy.deepcopy().
