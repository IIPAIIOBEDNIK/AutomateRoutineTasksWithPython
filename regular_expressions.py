...
# # Символ подстановки
# # Символ .(точка) в регулярных выражениях называется символ подстановки и представляет собой сокращенную форму
# # записи символьного класса, совпадающего с любым одиночным символом, за исключением символа новой строки.
# import re
# at_regex = re.compile(r'.at')
# print(at_regex.findall('The cat in the hat sat on the flat mat.'))
# # Символ подстановки соответствует только одному символу, поэтому в слове 'flat' была выбрана подстрока 'lat'




# # Поиск любого текста с помощью комбинации "точка - звездочка"
# # Символ . означает "любой одиночный символ, за исключением символа новой строки", а символ * означает "null или
# # более вхождений предыдущего символа"
# import re
# name_regex = re.compile(r'Имя: (.*) Фамилия: (.*)')
# mo = name_regex.search('Имя: Эл Фамилия: Свейгарт')
# print(mo.group(1))
# print(mo.group(2))
#
#
# # Комбинация .* работает в режиме жадного поиска: она всегда будет стремиться захватить как можно больше текста.
# # Чтобы заставить ее работать в нежадном режиме, дополните ее вопросительным знаком: .*?. Как и в случае фигурных
# # скобок, вопросительный знак означает использование нежадного поиска.
# non_greedy_regex = re.compile(r'<.*?>')
# mo = non_greedy_regex.search('<Приготовить мужу> ужин.>')
# print(mo.group())
#
# greedy_regex = re.compile(r'<.*>')
# mo = greedy_regex.search('<Приготовить мужу> ужин.>')
# print(mo.group())
# # Смысл обоих выражений можно описать так: "Найти открывающую угловую скобку, за которой следует произвольный текст,
# # завершающийся закрытием угловой скобкой". Однако в строке "<Приготовить мужу> ужин>" имеются две закрывающиеся
# # угловые скобки. В нежадной версии регулярного выражения Python ограничивается самой короткой из возможных строк:
# # "<Приготовить мужу>". В жадной версии Python стремится к тому, чтобы найти совпадение в виде как можно более
# # длинной строки из всех возможных: '<Приготовить мужу> ужин.>'




# # Поиск символов новой строки с помощью точки
# # Комбинации "точка - звездочка" будет соответствовать все, за исключением символа новой строки. Передав методу
# # re.compile() дополнительный аргумент re.DATALL, можно установить режим, при котором точке соответствуют все
# # символы, включая символ новой строки.
# import re
# no_new_line_regex = re.compile('.*')
# print(no_new_line_regex.search('Служить обществу. \nЗащищать невиноватых. \nСоблюдать закон.').group())
# new_line_regex = re.compile('.*', re.DOTALL)
# print(new_line_regex.search('Служить обществу. \nЗащищать невиноватых. \nСоблюдать закон.').group())
...
# { , m} - отсутствие или вплоть до m вхождений предшествующей
...
# \d, \w, \s - одиночный цифровой, алфавитно-цифровой и пробельный символ соответственно
# \D, \W, \S - одиночный символ, не являющийся цифровым, алфавитно-цифровым и пробельным соответственно
...
# # Обычно поиск регулярного выражения проводится с учетом регистра.
# import re
...
# # Иногда интересует лишь сам факт совпадения букв, независимо от их регистра. Можно включить режим, в котором
# # регистр букв игнорируется, передав методу re.compile() дополнительный аргумент re.IGNORECASE или re.I
# import re
# robocop = re.compile(r'робокоп', re.I)
# print(robocop.search('РобоКоп - это полицейский, частично человек, частично машина.').group())
# print(robocop.search('РОБОКОП защищает невинных.').group())
# print(robocop.search('Эл, почему в твоей книге по программированию так часто упоминается робокоп?').group())




# # Замена строк с помощью метода sub()
# # С помощью регулярных выражений можно не только находить заданные образцы текста, но и заменять их новым текстом.
# # У объектов Regex есть метод sub(), имеющий два аргумента. Первый аргумент - это строка, которая должна
# # подставляться вместо найденных совпадений. Второй аргумент - это строка, в которой выполняется поиск регулярного
# # выражения. Метод sub() возвращает строку, в которой выполнены замены.
# import re
# names_regex = re.compile(r'агент \w+', re.I)
# print(names_regex.sub('Цензура', 'Агент Алиса передала секретные документы. Агент Боб.'))


# # Иногда сам найденный текст должен включаться в строку замены. Для этого в первом аргументе метода sub() можно
# # использовать ссылки \1, \2, \3 и т.д. Предположим, вы хотите скрыть в тексте имена секретных агентов, показав
# # лишь первые буквы. Для этого можно воспользоваться регулярным выражением агент (\w)\w* и передать методу sub()
# # в качестве первого аргумента строку r'\1****'. Ссылка \1 в этой строке будет заменяться тем текстом, который
# # будет захвачен группой 1, т.е. группой (\w) регулярного выражения.
# import re
# agent_names_regex = re.compile(r'агент (\w)\w*', re.I)
# print(agent_names_regex.sub(r'\1****', 'Агент Алиса передала, что агент Ева знает: агент Боб - двойной агент.'))




# Работа со сложными регулярными выражениями
# С регулярными выражениями легко работать в случае простых текстовых шаблонов. Но для поиска более сложных шаблонов
# могут требоваться длинные и запутанные регулярные выражения. Один из способов внесения ясности в подобных ситуациях
# заключается в том, чтобы заставить метод re.compile() игнорировать пробелы и комментарии в строке регулярного
# выражения. Для этого следует передать методу re.compile() дополнительный аргумент re.VERBOSE.
# Предположим, имеется следующее головоломное регулярное выражение:
import re
phone_regex = re.compile(r'((\d{3}|\(\d{3}\))?(\s|-|\.)?\d{3}(\s|-|\.)\d{4}(\s*(ext|x|ext.)\s*\d{2,5})?)')


# Его можно разбить на несколько строк и снабдить комментариями, благодаря которым понять смысл регулярного выражения
...
# Обратите внимание на то, что строка регулярного выражения взята в тройные кавычки('''). Это позволило разбить текст
# на несколько строк, благодаря чему его стало намного легче читать.




# Комбинация констант re.IGNORECASE, re.DOTALL и re.VERBOSE
# Если вы хотите включить режим re.VERBOSE для добавления комментариев в регулярное выражение, и при этом необходимо
# игнорировать регистр символов с помощью константы re.IGNORECASE? К сожалению, у метода re.compile() только два
# аргумента. Это ограничение можно обойти, объединив константы re.IGNORECASE, re.DOTALL и re.VERBOSE с помощью
# символа |, который в данном контексте является оператором побитового ИЛИ. Таким образом, если требуется, чтобы
# регулярное выражение игнорировало регистр, а символам точки в нем соответствовали бы также символы новой строки,
# вызовите метод re.compile() следующим образом
import re
some_regex_value = re.compile('foo', re.IGNORECASE | re.DOTALL)
# А пример объединения всех трех констант во втором аргументе.
import re
